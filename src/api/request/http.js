...
if (process.env.NODE_ENV == 'development') {
    ...
    //   axios.defaults.baseURL = 'http://192.168.150.163:8733/zuul'//测试环境
    // axios.defaults.baseURL = "http://172.16.110.56:7201";
...
export const request = {
    ...
    async getc(url, data, config) {
        try {
            console.log('getc', { ...{ params: data }, ...config });
            let res = await axios.get(url, { ...{ params: data }, ...config });
            res = res.data;
            return new Promise((resolve, reject) => {
                if (res.code === 1 || res.code === 303) {
                    resolve(res);
    ...
    async postc(url, data, config) {
        try {
            let headers = { 'Content-Type': 'application/json;charset=UTF-8' };
            let res = await axios.post(url, data, { ...headers, ...config });
            res = res.data;
            return new Promise((resolve, reject) => {
                if (res.code === 1) {
                    resolve(res);
    ...
    // 自定义post请求
    async commonPost(url, params, _object = {}) {
        try {
            const res = await axios.post(url, params, _object);
