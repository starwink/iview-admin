<template>
    ...
    <date-picker prefix-class="ewell" type="datetime" :append-to-body="appendToBody" ref="ewell-date-picker"
        :open="show.panel" v-model="date" value-type="format" format="YYYY-MM-DD HH:mm" range :clearable="clearable"
        @focus="showPanel" @close="hidePanel" v-bind="$attrs" @change="change">
...
export default {
    ...
    methods: {
        setSectionDate(name) {
            ...
            let val = [null, null];
            let start = dayjs().format("YYYY-MM-DD 00:00");
            let end = dayjs().format("YYYY-MM-DD 23:59");
            switch (name) {
                case "all":
                    val = [null, null];
                    break;
