...
<template>
    <!-- 最小维度为年的时间框 -->
    <date-picker prefix-class="ewell" type="year" :append-to-body="appendToBody" ref="ewell-date-picker" :open="show.panel" v-model="date" value-type="format" format="YYYY-MM-DD" range @focus="showPanel" @close="hidePanel" v-bind="$attrs"  @change="changeEndDate">
    ...
    methods: {
        ...
        changeEndDate(dateRange) {
            ...
            if (dateRange[0] && dateRange[1]) {
                date = [
                    dateRange[0],
                    dayjs(dateRange[1]).endOf('year').format('YYYY-MM-DD')
                ]
            }
